...
def get_accounting_journal(entry):
    ...
    if applicable_rules:
        applicable_rules = sorted(
            [
                ...
                if rule.document_type in (entry.voucher_type, None)
            ],
        ...
        applicable_rules = [
            rule for rule in rules if rule.document_type == entry.voucher_type
        ]
    ...
    if not accounting_journal and [
        ...
        accounting_journal = [rule for rule in applicable_rules if not rule.condition][
            0
        ].name

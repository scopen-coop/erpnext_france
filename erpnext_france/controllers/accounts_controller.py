...
def get_advance_entries(doc):
    ...
    order_list = list(
        set(d.get(order_field) for d in doc["items"] if d.get(order_field))
    )
...
def update_against_document_in_jv(doc):
    ...
    for d in doc.get("advances"):
        ...
        down_payment = (
            cint(
                frappe.db.get_value(d.reference_type, d.reference_name, "down_payment")
            )
...
def set_total_advance_paid(doc):
    ...
    advance_query = (
        frappe.qb.from_(ple)
        .select(
            ple.account_currency, Abs(Sum(ple.amount_in_account_currency)).as_("amount")
        )
        .where(
            (ple.party == party) & (ple.docstatus == 1) & (ple.company == doc.company)
        )
    )
    ...
    if advance:
        ...
        frappe.db.set_value(
            doc.doctype, doc.name, "party_account_currency", advance.account_currency
        )
        ...
        if account_currency != doc.company_currency:
            frappe.throw(
                _("Currency for {0} must be {1}").format(
                    gain_loss_account, doc.company_currency
                )
            )
            ...
            doc.get_gl_dict(
                {
                    ...
                    or get_default_cost_center(doc.company),
                    "project": doc.project,
            ...
            doc.get_gl_dict(
                {
                    ...
                    dr_or_cr + "_in_account_currency": flt(
                        abs(d.exchange_gain_loss) / doc.conversion_rate
                    ),
                    dr_or_cr: abs(d.exchange_gain_loss),
                    "cost_center": doc.cost_center,
                    "project": doc.project,

...
class DataExporter:
	def __init__(
		...
		self.journal_code = ""
		self.account_code_length = frappe.db.get_value("Company", self.company, "account_code_length") or 0
	...
	def build_response(self):
		...
		if self.journal_code == "" or self.journal_code is None:
			frappe.respond_as_web_page(
				...
				_("ERPNext France - Please Set Accounting Jounal For Purchases and Sales"),
	...
	def add_data(self):
		...
		sql = (
			...
			.left_join(cust)
			.on((gl.party == cust.name) & (gl.party_type == 'Customer'))
	...
	def add_row_ciel(self, doc):
		...
		journal_code = f"{self.journal_code:<2s}"

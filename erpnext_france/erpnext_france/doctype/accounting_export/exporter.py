...
class DataExporter:
    def __init__(
        ...
        self.file_format = frappe.db.get_value(
    ...
    def build_response(self):
        ...
        if self.journal_code == "" or self.journal_code is None:
            frappe.respond_as_web_page(
                _("ERPNext France - Accounting Export Not Possible"),
                _(
                    "ERPNext France - Please Set Accounting Jounal For Purchases and Sales"
                ),
    ...
    def get_jounal_code(self):
        try:
            ...
            frappe.throw(_("ERPNext France - Error : {0}").format(e))
    ...
    def add_data(self):
        ...
        sql = (
            ...
            .where(
                (self.company == party_acc_supp.company)
                | (party_acc_supp.company.isnull())
            )
            .where(
                (self.company == party_acc_cust.company)
                | (party_acc_cust.company.isnull())
            )
        )
        ...
        if self.file_format == "CIEL":
            for doc in self.data:
                ...
                _set_export_date(
                    self.accounting_document, doc.voucher_no, self.export_date
                )

...
class SelectInTableDialog {
  ...
  make() {
    ...
    this.dialog = new frappe.ui.Dialog({
      ...
      primary_action: () => {
        ...
        const selected_documents = [...this.get_checked_values()];
  ...
  render_datatable() {
    if (!this.datatable) {
      ...
      setTimeout(() => {
        ...
        this.datatable.refresh(this.get_datatable_rows());
  ...
  setup_datatable() {
    const header_columns = this.get_datatable_columns();
    const rows = this.get_datatable_rows();
    this.$child_wrapper = this.dialog.fields_dict.child_selection_area.$wrapper;
    this.$child_wrapper.addClass("my-3");

    this.datatable = new frappe.DataTable(this.$child_wrapper.get(0), {
  ...
  get_selected_child_names() {
    if (!this.datatable || !this.datatable.datamanager.rows.length) return [];
  ...
  async perform_search(args) {
    ...
    const more =
      res.values.length && res.values.length > this.page_length ? 1 : 0;

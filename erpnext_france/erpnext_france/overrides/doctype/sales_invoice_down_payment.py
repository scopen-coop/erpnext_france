...
class SalesInvoiceDownPayment(SalesInvoice):
    ...
    def validate(self):
        ...
        if (
            ...
            and len([x.sales_order for x in self.get("items")]) > 1
        ):
            frappe.throw(
                _(
                    "Down payment invoices can only be made against a single sales order."
                )
            )
    ...
    def make_down_payment_final_invoice_entries(self, gl_entries):
        ...
        for d in self.get("advances"):
            ...
            down_payment_accounts = [
                entry["against"]
                for entry in down_payment_entries
                if entry["account"] == self.debit_to
            ]
            ...
            for down_payment_entry in down_payment_entries:
                if down_payment_entry["account"] in down_payment_accounts and not [
                    x
                    for x in gl_entries
                    if x["account"] == down_payment_entry["account"]
                ]:
                    gl_entries.append(
                        self.get_gl_dict(
                            {
        ...
        for item in self.get("items"):
            ...
            if item.is_fixed_asset:
                ...
                income_account = (
                    item.income_account
                    if (
                        not item.enable_deferred_revenue
                        or self.is_return
                        or self.is_down_payment_invoice
                    )

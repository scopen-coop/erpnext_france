...
def validate(doc, method):
	...
	if doc.customer and not doc.get("__islocal"):
		...
		if customer.get("default_mode_of_payment_sepa") and not doc.get("mode_of_payment"):
			# Map to actual mode of payment
			mode_mapping = {"Direct Debit": "SEPA Direct Debit", "Credit Transfer": "SEPA Credit Transfer"}
			mapped_mode = mode_mapping.get(customer.default_mode_of_payment_sepa)

...
def get_result(filters, account_details):
    ...
    data = gl.get_data_with_opening_closing(
...
def get_gl_entries(filters, accounting_dimensions):
    ...
    if accounting_dimensions:
        for dimension in accounting_dimensions:
            gl_entries = gl_entries.select(gle[dimension])

    if filters.get("show_remarks"):
        gl_entries = gl_entries.select(gle.remarks)

    order_by_statement = gl_entries.orderby(gle.posting_date, gle.account, gle.creation)

    if filters.get("include_dimensions"):
        order_by_statement = gl_entries.orderby(gle.posting_date, gle.creation)
    ...
    if filters.get("group_by") == "Group by Account":
        order_by_statement = gl_entries.orderby(
            gle.account, gle.posting_date, gle.creation
        )
    ...
    if filters.get("presentation_currency"):
        return gl.convert_to_presentation_currency(
            gl_entries.run(as_dict=1), currency_map
        )
    else:
...
def get_conditions(filters, gle, gl_entries):
    ...
    if filters.get("against_voucher_no"):
        filters_query = filters_query.where(
            gle.against_voucher_no == filters.against_voucher_no
        )
    ...
    if filters.get("voucher_no_not_in"):
        filters_query = filters_query.where(
            gle.voucher_no.notin(filters.voucher_no_not_in)
        )
    ...
    if not (
        ...
        filters_query = filters_query.where(
            (gle.posting_date >= filters.from_date) | (gle.is_opening == "Yes")
        )

    filters_query = filters_query.where(
        (gle.posting_date <= filters.to_date) | (gle.is_opening == "Yes")
    )
    ...
    if filters.get("include_default_book_entries"):
        if filters.get("finance_book"):
            if filters.get("company_fb") and cstr(filters.get("finance_book")) != cstr(
                ...
                filters_query = filters_query.where(
                    (gle.finance_book.isin(filters.finance_book))
                    | (gle.finance_book.isnull())
                )
        else:
            filters_query = filters_query.where(
                (gle.finance_book.isin(("", filters.company_fb)))
                | (gle.finance_book.isnull())
            )
    else:
        if filters.get("finance_book"):

...
class PaymentEntryDownPayment(PaymentEntry):
    ...
    def update_unreconciled_amount(self):
        ...
        self.unreconciled_amount = flt(self.unreconciled_from_amount) + flt(
            self.unreconciled_to_amount
        )
    ...
    def get_cash_flow_journal(self, account):  # Erpnext France
        ...
        applicable_rules = [
            rule
            for rule in rules
            if (rule.document_type == self.doctype and rule.account == account)
        ]
    ...
    def update_advance_paid(self):
        if self.payment_type in ("Receive", "Pay") and self.party:
            for d in self.get("references"):
                ...
                if (
                    d.allocated_amount and d.reference_doctype == "Sales Invoice"
                ):  # Erpnext France
